...
class ListResult(list, PickleMixin):
    ...
    def plot(self, ucrp=False, bah=False, bcrp=False, olmar=False, bnn=False, corn=False,crp=False,cwmr=False,eg=False,kelly=False, ons=False, pamr=False, rmr=False, up=False, residual=False, capm_residual=False, assets=False, **kwargs):
        ...
        # add BCRP
        if bcrp:
            from .algos import BCRP
            bcrp_algo = BCRP().run(self[0].X.cumprod())
            bcrp_algo.fee = self[0].fee
            d['BCRP'] = bcrp_algo.equity
            d[['BCRP']].plot(**kwargs)

        # add Olmar
        if olmar:
            from .algos import OLMAR
            olmar_algo = OLMAR().run(self[0].X.cumprod())
            olmar_algo.fee = self[0].fee
            d['OLMAR'] = olmar_algo.equity
            d[['OLMAR']].plot(**kwargs)

        # add BNN
        if bnn:
            from .algos import BNN
            BNN_algo = BNN().run(self[0].X.cumprod())
            BNN_algo.fee = self[0].fee
            d['BNN'] = BNN_algo.equity
            d[['BNN']].plot(**kwargs)

        # add CORN
        if corn:
            from .algos import CORN
            CORN_algo = CORN().run(self[0].X.cumprod())
            CORN_algo.fee = self[0].fee
            d['CORN'] = CORN_algo.equity
            d[['CORN']].plot(**kwargs)
        ...
        if crp:
            ...
            CRP_algo = CRP().run(self[0].X.cumprod())
            CRP_algo.fee = self[0].fee
            d['CRP'] = CRP_algo.equity
            d[['CRP']].plot(**kwargs)
            # add CRP
        if cwmr:
            from .algos import CWMR
            CWMR_algo = CWMR().run(self[0].X.cumprod())
            CWMR_algo.fee = self[0].fee
            d['CWMR'] = CWMR_algo.equity
            d[['CWMR']].plot(**kwargs)

        if eg:
            from .algos import EG
            EG_algo = EG().run(self[0].X.cumprod())
            EG_algo.fee = self[0].fee
            d['EG'] = EG_algo.equity
            d[['EG']].plot(**kwargs)

        if kelly:
            from .algos import Kelly
            Kelly_algo = Kelly().run(self[0].X.cumprod())
            Kelly_algo.fee = self[0].fee
            d['Kelly'] = Kelly_algo.equity
            d[['Kelly']].plot(**kwargs)

        if ons:
            from .algos import ONS
            ONS_algo = ONS().run(self[0].X.cumprod())
            ONS_algo.fee = self[0].fee
            d['ONS'] = ONS_algo.equity
            d[['ONS']].plot(**kwargs)

        if pamr:
            from .algos import PAMR
            PAMR_algo = PAMR().run(self[0].X.cumprod())
            PAMR_algo.fee = self[0].fee
            d['PAMR'] = PAMR_algo.equity
            d[['PAMR']].plot(**kwargs)

        if rmr:
            from .algos import RMR
            RMR_algo = RMR().run(self[0].X.cumprod())
            RMR_algo.fee = self[0].fee
            d['RMR'] = RMR_algo.equity
            d[['RMR']].plot(**kwargs)

        if up:
            from .algos import UP
            UP_algo = UP().run(self[0].X.cumprod())
            UP_algo.fee = self[0].fee
            d['UP'] = UP_algo.equity
            d[['UP']].plot(**kwargs)
